...
public class CompAlu {
	public void grafica() {
        ...
        for(int i=0; i<alumno.length; i++) {
        	if(alumno[i]!=null) {
	        	if(alumno[i].getGenero().equals("F")) {
	        		m++;
	        	}else {
	        		h++;
	        	}
	...
	public void tabla() {
		...
		for(int i=0; i<alumno.length; i++) {
			if(alumno[i]!=null) {
				profesores[i][0]=String.valueOf(alumno[i].getCodigo());
				profesores[i][1]=alumno[i].getNombre();
				profesores[i][2]=alumno[i].getApellido();
				profesores[i][3]=alumno[i].getCorreo();
				profesores[i][4]=alumno[i].getGenero();
			}
		}
